...
fun RestoreNovaBackupScreen(
    ...
    PreferenceLayout(
        ...
        when (uiState) {
            is RestoreNovaBackupUiState.Success -> RestoreNovaBackupContent(uiState.converter, uiState.info)

            is RestoreNovaBackupUiState.Loading -> {
                Box(modifier = Modifier.fillMaxSize()) {
                    CircularProgressIndicator(modifier = Modifier.align(Alignment.Center))
                }
            }

            is RestoreNovaBackupUiState.Error -> {
...
@Composable
fun ColumnScope.RestoreNovaBackupContent(
    converter: NovaBackupConverter,
    info: NovaBackupInfo,
    modifier: Modifier = Modifier,
) {
    val context = LocalContext.current
    ...
    fun restore() {
        ...
        scope.launch {
            restoring = true
            try {
                converter.convertAndRestore(info)
    ...
    PreferenceGroup(
        ...
        Text(
            text = stringResource(R.string.nova_grid_info, info.columns, info.rows, info.hotseatCount),
            modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
        )
        Text(
            text = stringResource(R.string.nova_item_count, info.itemCount),
            modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
        )
        if (!info.iconPackPackage.isNullOrEmpty()) {
            Text(
                text = stringResource(R.string.nova_icon_pack, info.iconPackPackage),
                modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
            )
        }
    }
